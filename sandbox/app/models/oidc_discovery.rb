...
class OidcDiscovery
  class << self
    def config
      Rails.cache.fetch("oidc:discovery:#{Settings.oidc_issuer}", expires_in: 1.hour) do
        result = OpenIDConnect::Discovery::Provider::Config.discover!(Settings.oidc_issuer)
        {
